...
1. `runtime-snapshot-service`
   负责发布后生成运行时快照，按页面、组织范围、角色和版本投放给运行面，并下发页面字段元数据、元素类型、locator、执行方式等运行信息。
...
3. `rule-runtime-client`
   拉取运行时快照、执行本地规则判定、管理页面停留期间的字段基线状态、字段终态变化和接口缓存。
4. `prompt-renderer`
   负责静默提示、浮窗提示、关闭行为以及执行方式要求的提示确认交互。
5. `scene-trigger-client`
   负责按执行方式承接自动触发、提示确认触发、悬浮按钮二次触发以及执行实例生命周期。
6. `preview-renderer`
