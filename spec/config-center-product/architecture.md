...
3. JSSDK 以“本地规则运行 + 强约束”为原则，后端负责运行时快照发布、数据代理、审计留痕和治理校验。
4. 治理日志和运行日志分流采集，通过 MQ 异步削峰，避免阻塞现场操作。
5. JSSDK 通过 CDN 分发时，CDN 只改善资源分发与缓存命中，不降低浏览器本地解析、执行与 DOM 处理成本，因此运行面必须采用“统一宿主 + 分包加载”而非提示与作业全量大包。
...
5. `preprocessor-service`
   维护内置预处理器与受控扩展元数据。
6. `sdk-release-service`
   维护 SDK 制品版本、版本槽位、菜单版本策略、机构灰度范围和版本兼容校验。
7. `publish-validation-service`
   负责发布前强校验、冲突扫描、依赖检查和例外确认。
8. `role-permission-service`
   负责角色模板、组织范围、操作类型授权、人员绑定和权限校验。

#### 5.2.2 运行面服务

1. `runtime-snapshot-service`
   负责发布后生成运行时快照，按页面、组织范围、角色和版本投放给运行面，并下发页面字段元数据、元素类型、locator、执行方式等运行信息；对外输出时需拆分为 `manifest`、`sdk-release-index`、`page-index`、`page-config`、`scene-config` 等可缓存静态资源。
2. `runtime-data-proxy-service`
   负责运行时数据代理、鉴权、限流、缓存策略下发和接口调用审计。
...
5. 快照需包含页面字段元数据、元素类型、locator、字段依赖索引、执行方式配置和必要的字段提取策略。
...
3. 技术人员负责发布不可变 SDK 制品版本并维护 `stable`、`gray-a` 等版本槽位；业务人员只选择槽位，不直接操作具体版本号。
4. 业务侧灰度范围简化到机构维度，默认不开放人员级灰度；运行时按 `orgId` 命中菜单灰度策略。
