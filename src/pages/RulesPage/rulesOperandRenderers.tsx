...
import { OPERAND_PILL_WIDTH, contextOptions, pageFieldOptions, resolveOperandSummary } from "./rulesPageShared";
import type { InterfaceInputParamDraft, OperandDraft, OperandSide, SelectedOperand } from "./rulesPageShared";
...
export function InterfaceInputValueEditor({ param, selectedInterfaceInputConfig, updateInterfaceInputValue }: InterfaceInputValueEditorProps) {
  const value = selectedInterfaceInputConfig[param.name] ?? param.sourceValue ?? "";
  if (param.sourceType === "PAGE_ELEMENT") return <Select showSearch allowClear placeholder="选择页面元素" value={value || undefined} options={pageFieldOptions.map((item) => ({ label: item, value: item }))} onChange={(next) => updateInterfaceInputValue(param.name, (next as string) ?? "")} />;
